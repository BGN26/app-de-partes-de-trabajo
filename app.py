...
# Configuración API de Google (Recuerda poner tu API KEY real)
...
def ocr_camara():
    if request.method == 'POST':
        ...
        ruta_procesada = os.path.join(app.config['UPLOAD_FOLDER'], 'procesada_' + filename)

        # 1. PROCESAMIENTO DE IMAGEN (OPENCV)
        try:
            img = cv2.imread(filepath)
            gris = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            gris = cv2.resize(gris, None, fx=1.5, fy=1.5, interpolation=cv2.INTER_LINEAR)
        ...
        # 2. CONEXIÓN MULTIMODAL CON GEMINI
        datos_ia = {
            "cliente": "",
            "descripcion_trabajo": "",
            "tipo de servicio": "",
        ...
        try:
            ...
            with Image.open(ruta_procesada) as img_procesada:
                # OBLIGAMOS al modelo a devolver siempre un formato application/json
            ...
            # Como forzamos application/json, no hace falta limpiar comillas invertidas ni markdown
            ...
            # 3. LÓGICA DE DETECCIÓN DIFUSA (FUZZY MATCHING AVANZADO)

        def detectar_entidad_por_texto(lista_objetos, texto_buscar, umbral_seguro=85, umbral_dudoso=50):
            if not texto_buscar:
                return None, False
            mejor_id = None
            mejor_puntuacion = 0
            # Convertimos a string para evitar errores si la IA devuelve números
        ...
        cliente_id, cliente_dudoso = detectar_entidad_por_texto(clientes, datos_ia.get("cliente", ""))
        servicio_id, servicio_dudoso = detectar_entidad_por_texto(servicios, datos_ia.get("tipo de servicio", ""))

        # NUEVO: Procesamos los materiales aquí en Python, no en JavaScript
        materiales_procesados = []
        for mat_ia in datos_ia.get("materiales", []):
            nombre_mat = mat_ia.get("nombre", "")
            # Usamos un umbral más bajo (50) para que sea más permisivo con los nombres de artículos
            mat_id, mat_dudoso = detectar_entidad_por_texto(materiales, nombre_mat, umbral_seguro=85, umbral_dudoso=50)

            if mat_id:
                # Si encontramos coincidencia, guardamos directamente el ID de la base de datos
                materiales_procesados.append({
                    "id": mat_id,
                    "cantidad": int(mat_ia.get("cantidad", 1))
                })

        flash('Imagen analizada directamente por la IA con éxito.', 'success')

        return render_template('form_parte.html',
                               clientes=clientes,
                               servicios=servicios,
...
def ver_parte(id):
    ...
    # Obtenemos los datos vinculados de manera segura mediante consultas explícitas
    ...
    # Construimos una lista limpia de materiales para la plantilla
    ...
    return render_template('ver_parte.html',
                           parte=parte,
                           cliente=cliente,
                           servicio=servicio,
...
def generar_pdf(id):
    ...
    # Inicializamos el generador de PDF
    pdf = FPDF()
    pdf.add_page()
    pdf.set_margins(15, 15, 15)

    # Encabezado de la empresa (Usamos helvetica y nueva sintaxis de salto de línea)
    pdf.set_font("helvetica", 'B', 18)
    pdf.cell(0, 10, "FACTURA / PARTE DE TRABAJO", new_x="LMARGIN", new_y="NEXT", align='C')
    pdf.set_font("helvetica", '', 10)
    pdf.cell(0, 5, f"Documento de Control Interno N#: {parte.id}", new_x="LMARGIN", new_y="NEXT", align='C')
    pdf.ln(10)
    ...
    pdf.cell(0, 6, f"Nombre / Razón Social: {cliente.nombre if cliente else 'No asignado'}", new_x="LMARGIN",
             new_y="NEXT")
    if cliente:
    ...
    if servicio:
        ...
        pdf.cell(35, 8, f"{total_mano_obra} EUR", border=1, align='R', new_x="LMARGIN", new_y="NEXT")

    # Filas siguientes: Materiales utilizados
    for item in materiales_rel:
        prod = Producto.query.get(item.producto_id)
        if prod:
            ...
            pdf.cell(35, 8, f"{subtotal_mat} EUR", border=1, align='R', new_x="LMARGIN", new_y="NEXT")

    # Fila de Total final
    pdf.ln(4)
    pdf.set_font("helvetica", 'B', 13)
    pdf.cell(145, 10, "TOTAL IMPORTE:", align='R')
    pdf.cell(35, 10, f"{parte.total_factura} EUR", align='R', new_x="LMARGIN", new_y="NEXT")

    # ---------------------------------------------------------
    # LA SOLUCIÓN AL ERROR ESTÁ AQUÍ ABAJO: bytes(pdf.output())
    # ---------------------------------------------------------
    response = make_response(bytes(pdf.output()))
    response.headers['Content-Type'] = 'application/pdf'
    response.headers['Content-Disposition'] = f'inline; filename=factura_parte_{parte.id}.pdf'
    return response
